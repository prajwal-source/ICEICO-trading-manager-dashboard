...
import ModifyPositionPopup from './popups/ModifyPostionPopup';
...
const columns = [
    ...
    { header: "Number", field: "number" },
    {
        ...
        render: (value) => {
            ...
            const isBuyLimit = normalized === "buy(limit)";
            ...
            return (
                <div className="flex items-center gap-1">
                    {isBuy && (
                        ...
                        <ArrowUp size={14} className="text-green-600 font-bold" />
                    )}

                    {isSell && (
                        <ArrowDown size={14} className="text-red-600 font-bold" />
                    )}
                    {isBuyLimit && (
                        <TrendingUp size={14} className="text-green-600 font-bold" />
                    )}
...
function Position() {
    const [modifyPosition, setModifyPosition] = useState(false);
    return (
        <>
            {/* DataTable */}
            <DataTable
                ...
                onRowDoubleClick={(row) => {
                    setModifyPosition(true);
                }}
                showEditColumn
                showDeleteColumn
                onEditClick={(row) => setModifyPosition(true)}
            ...
            <ModifyPositionPopup
                isOpen={modifyPosition}
                onClose={() => setModifyPosition(false)}
            />
        </>
    )
}
